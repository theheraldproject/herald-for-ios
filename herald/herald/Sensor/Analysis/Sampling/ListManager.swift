...
public class ListManager<T: DoubleValue> {
    private let queue = DispatchQueue(label: "Sensor.Analysis.Sampling.ListManager")
    private let listSize: Int
    private var map: [SampledID:SampleList<T>] = [:]
    
    public init(_ listSize: Int) {
        self.listSize = listSize
    }
    
    public func list(_ listFor: SampledID) -> SampleList<T> {
        queue.sync {
            if let list = map[listFor] {
                return list
            } else {
                let list = SampleList<T>(listSize)
                map[listFor] = list
                return list
            }
        }
    ...
    public func push(_ sampledID: SampledID, _ sample: Sample<T>) {
