...
class RangesTests: XCTestCase {
    ...
    func test_ranges_filter_typed() {
        let workingAge = InRange<Int32>(18, 65)
        let ages = SampleList<Int32>(5)
        ages.push(secondsSinceUnixEpoch: 10, value: Int32(12))
        ages.push(secondsSinceUnixEpoch: 20, value: Int32(14))
        ages.push(secondsSinceUnixEpoch: 30, value: Int32(19))
    ...
    func test_ranges_filter_multi() {
        let workingAge = InRange<Int32>(18, 65)
        let over21 = GreaterThan<Int32>(21)
        let ages = SampleList<Int32>(5)
        ages.push(secondsSinceUnixEpoch: 10, value: Int32(12))
        ages.push(secondsSinceUnixEpoch: 20, value: Int32(14))
        ages.push(secondsSinceUnixEpoch: 30, value: Int32(19))
    ...
    func test_ranges_filter_multi_rssisamples() {
        let valid = InRange<RSSI>(-99, -10)
        let strong = LessThan<RSSI>(-59)
        let sl = SampleList<RSSI>(5)
        sl.push(secondsSinceUnixEpoch: 1234, value: RSSI(-9))
        sl.push(secondsSinceUnixEpoch: 1244, value: RSSI(-60))
        sl.push(secondsSinceUnixEpoch: 1265, value: RSSI(-58))
    ...
    func test_ranges_filter_multi_summarise() {
        ...
        let mean = Mean<RSSI>()
        let mode = Mode<RSSI>()
        let variance = Variance<RSSI>()
        let median = Median<RSSI>()
        let gaussian = Gaussian<RSSI>()
        
        // values = -60, -62, -68, -68
        let summary = values.aggregate([mean, mode, variance, median, gaussian])
        XCTAssertEqual(summary.get(Mean<RSSI>.self), -64.5)
        XCTAssertEqual(summary.get(Mode<RSSI>.self), -68)
        XCTAssertEqual(summary.get(Variance<RSSI>.self), 17)
        XCTAssertEqual(summary.get(Median<RSSI>.self), -65)
        XCTAssertEqual(summary.get(Gaussian<RSSI>.self), -64.5)
    ...
    func test_ranges_filter_multi_since_summarise() {
        let valid = InRange<RSSI>(-99, -10)
        let strong = LessThan<RSSI>(-59)
        let afterPoint = Since<RSSI>(1245);
        let sl = SampleList<RSSI>(20)
        sl.push(secondsSinceUnixEpoch: 1234, value: RSSI(-9))
        sl.push(secondsSinceUnixEpoch: 1244, value: RSSI(-60))
        sl.push(secondsSinceUnixEpoch: 1265, value: RSSI(-58))
        ...
        // values = -60, -68, -68
        let summary = values.aggregate([mean, mode, variance, median, gaussian])
        XCTAssertEqual(summary.get(Mean<RSSI>.self), -66)
        XCTAssertEqual(summary.get(Mode<RSSI>.self), -68)
        XCTAssertEqual(summary.get(Variance<RSSI>.self), 12)
        XCTAssertEqual(summary.get(Median<RSSI>.self), -68)
        XCTAssertEqual(summary.get(Gaussian<RSSI>.self), -66)
    ...
    func test_ranges_distance_aggregate() {
        ...
        let modeValue = summary.get(Mode<RSSI>.self)!
        ...
        let toDistance = FowlerBasic<RSSI>(intercept: -50, coefficient: -24)
        ...
        XCTAssertEqual(distance.get(FowlerBasic<RSSI>.self)!, 5.6235, accuracy: 0.0005)
    }
    
    func test_ranges_risk_aggregate() {
        // First we simulate a list of actual distance samples over time, using a vector of pairs
        var sourceDistances: [Sample<Distance>] = []
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1235, value: Distance(5.5)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1240, value: Distance(4.7)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1245, value: Distance(3.9)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1250, value: Distance(3.2)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1255, value: Distance(2.2)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1260, value: Distance(1.9)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1265, value: Distance(1.0)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1270, value: Distance(1.3)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1275, value: Distance(2.0)))
        sourceDistances.append(Sample<Distance>(secondsSinceUnixEpoch: 1280, value: Distance(2.2)))

        // The below would be in your aggregate handling code...
        let distanceList = SampleList<Distance>(2)
        ...
        let riskScorer = RiskAggregationBasic<Distance>(
            timeScale: timeScale,
        ...
        for sourceDistance in sourceDistances {
            ...
            let riskSlice: Summary<Distance> = distanceList.aggregate([riskScorer])
            ...
            interScore = riskSlice.get(RiskAggregationBasic<Distance>.self)!
