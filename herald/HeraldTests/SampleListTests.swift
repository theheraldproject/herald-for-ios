...
class SampleListTests: XCTestCase {
    ...
    func test_sample_basic() {
        let s = Sample<RSSI>(secondsSinceUnixEpoch: 1234, value: RSSI(-55))
    ...
    func test_sample_from_parts() {
        let s = Sample<RSSI>(secondsSinceUnixEpoch: 1234, value: RSSI(-55))
        let s2 = Sample<RSSI>(taken: s.taken, value: s.value)
    ...
    func test_sample_from_parts_deep() {
        let s = Sample<RSSI>(secondsSinceUnixEpoch: 1234, value: RSSI(-55))
        let s2 = Sample<RSSI>(taken: Date(timeIntervalSince1970: s.taken.timeIntervalSince1970), value: s.value)
        XCTAssertEqual(s2.taken.secondsSinceUnixEpoch, 1234)
        XCTAssertEqual(s2.value.value, -55)
    }
    ...
    func test_sample_init() {
        let sample = Sample<RSSI>(secondsSinceUnixEpoch: 10, value: RSSI(-55))
        XCTAssertEqual(sample.taken.secondsSinceUnixEpoch, 10)
        XCTAssertEqual(sample.value.value, -55)
    }

    func test_samplelist_init_list() {
        let sample1 = Sample<RSSI>(secondsSinceUnixEpoch: 10, value: RSSI(-55))
        let sample2 = Sample<RSSI>(secondsSinceUnixEpoch: 20, value: RSSI(-65))
        let sample3 = Sample<RSSI>(secondsSinceUnixEpoch: 30, value: RSSI(-75))
        let sl = SampleList<RSSI>(3, samples: [sample1, sample2, sample3])
    ...
    func test_samplelist_init_deduced() {
        ...
        let sample2 = Sample<RSSI>(secondsSinceUnixEpoch: 20, value: RSSI(-65))
        let sample3 = Sample<RSSI>(secondsSinceUnixEpoch: 30, value: RSSI(-75))
        let sl = SampleList<RSSI>(samples: [sample1, sample2, sample3])
        XCTAssertEqual(sl.get(0)!.taken.secondsSinceUnixEpoch, 10)
        XCTAssertEqual(sl.get(0)!.value.value, -55)
        XCTAssertEqual(sl.get(1)!.taken.secondsSinceUnixEpoch, 20)
        XCTAssertEqual(sl.get(1)!.value.value, -65)
    ...
    func test_samplelist_init_alldeduced() {
        let sl = SampleList<RSSI>(samples: [
                                    Sample<RSSI>(secondsSinceUnixEpoch: 10, value: RSSI(-55)),
                                    Sample<RSSI>(secondsSinceUnixEpoch: 20, value: RSSI(-65)),
                                    Sample<RSSI>(secondsSinceUnixEpoch: 30, value: RSSI(-75))])
        XCTAssertEqual(sl.get(0)!.taken.secondsSinceUnixEpoch, 10)
        XCTAssertEqual(sl.get(0)!.value.value, -55)
        XCTAssertEqual(sl.get(1)!.taken.secondsSinceUnixEpoch, 20)
        XCTAssertEqual(sl.get(1)!.value.value, -65)
